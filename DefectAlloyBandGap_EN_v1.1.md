...
**Version**: v1.0 preprint (2026-08-04)
...
- **Defect compounds** (e.g., Ir0.67Se2) → endpoint = parent compound (not in current index)
...
| Metric | Value |
|---|---|
| Matched systems | 274 / 549 (50%) |
| Mean deviation | **+0.727 eV** |
| Median deviation | **+0.804 eV** |
| MAE | 0.865 eV |
| **Underestimation rate** | **81%** |
...
**dev = 0.714 × E_gap(expt) + 0.232 × (phys. gapless endpoint) + 0.401 × (comput. gapless endpoint) − 0.402**

MAE = 0.275 eV, R² = 0.792 (n = 274). Band-gap-only baseline: MAE = 0.314 eV.

## 5. Forward predictions (unresolved 275 systems)
...
2. Endpoint interpolation underestimates defect/alloy gaps in **81%** of 274 gold-standard systems; the error grows monotonically with the alloy gap (100% underestimation above 2 eV) and depends on endpoint quality (physically vs computationally gapless endpoints).
3. A falsifiable hypothesis framework survives in-sample falsification of half its predictions and yields a correction model with **R² = 0.79** (MAE 0.28 eV).
